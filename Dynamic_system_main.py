...
from Dynamic_system_charging_station import chargingstation
from Dynamic_system_vehicle import AMR
from Dynamic_system_production_station import ProductionStation
from Dynamic_system_supervisor import Supervisor
...
class ProductionModel(Model):
    ...
    def __init__(self,timesteps,no_AMR,station_inf,product_inf,charging_inf
                 ...
                 total_task_list,closed_loop_route,check_time,warmup):
        ...
        for i in range(1,no_AMR+1):
            amr = AMR(self.id+i,self,warehouse_inf,closed_loop_route,check_time)
            self.AMR.append(amr)
        self.id += i
...
buffer_threshold = 0.35
product_inf = [(79,167, 10*size, buffer_threshold ),
               (27,69,  4*size , buffer_threshold ),
               (62,149, 9*size , buffer_threshold ),
               (9,21,   3*size , buffer_threshold ),
               (30,101, 6*size , buffer_threshold )]
...
for k in range(nrRuns):
    ...
    for i in range(len(state_over_time[k])):
        ...
        amr_state_over_time[k][i].columns = ['Idle','Busy','Charging',' Broken','waiting']
...
parameters = [no_AMR, warmup, size, buffer_threshold, check_time]
parameters = pd.DataFrame(parameters)

with pd.ExcelWriter(f"Advanced_output{no_AMR},{size}.xlsx") as writer:
